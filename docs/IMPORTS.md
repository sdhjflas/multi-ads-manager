...
Registered [test waves](EXPERIMENTS.md) use this same target format. Copy source IDs and definitions exactly from the setup sheet. A registered ID cannot be repurposed for another candidate. Import the full fixed test window for both campaign and arms; corrected rows refresh the comparison and flag affected saved findings. Target imports never add their spend again to portfolio totals.
