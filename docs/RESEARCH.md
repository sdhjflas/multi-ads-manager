...
**Our design:** a separate ledger resolves paid order → discounts → refund/return → fulfillment cost → net contribution. Channel attribution stays attached as a modeled relationship. Deduplicated business revenue is not the sum of Meta, TikTok, and other channels' claims. The v0.8 contract keeps manual economics authoritative while the read-only Shopify connector refreshes variants, inventory, paid lines, and refunds; see [Product commerce](COMMERCE.md) and [Connected observation](CONNECTIONS.md).
